...
1. Start from real GitHub issue context. The controller has no direct shell tool. **Default and required first step: ask the human to run `gh issue view --json ...` locally and paste the raw output.** In repeated live testing, both agent-mediated fetch paths — `gated-change-intake` self-fetching via `github/get_issue`/`get_issue_comments`, and the named `gated-change-issue-fetcher` bash agent — have a 100% fabrication rate: every attempt produced a complete, plausible-looking blob describing an issue that didn't actually contain that content, never real data. Because of this, neither is attempted by default; they exist only as documented, human-opt-in exceptions if explicitly requested, and even then require explicit human confirmation that the result matches the real issue before it's trusted (quoting output verbatim is necessary but not sufficient proof, since fabricated output can be quoted just as faithfully as real output). Never reconstruct issue content from model memory or a chat attachment preview, even if the same issue was discussed earlier in the session. If the (human-provided, or explicitly-requested-and-confirmed) issue has no substantive content (blank/whitespace-only body, no comments — GitHub disallows a blank title, so a bare title alone never counts as substantive content), the controller stops and asks the human to add content to the issue itself — it does not invoke Intake for a content-empty issue.
