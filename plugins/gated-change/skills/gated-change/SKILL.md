...
1. Start from real GitHub issue context, fetched with a deterministic `gh issue view --json ...` command — never reconstructed from model memory or a chat attachment preview, even if the same issue was discussed earlier in the session. If the fetched issue has no substantive content (blank/whitespace-only body, no comments — GitHub disallows a blank title, so a bare title alone never counts as substantive content), the controller stops and asks the human to add content to the issue itself — it does not invoke Intake for a content-empty issue.
...
- Anti-fabrication is not just an instruction, it needs a forcing function: the controller must quote the exact raw JSON its fetch tool call returned, verbatim, before saying anything about the issue's content or readiness. A prose description of "what the issue contains" with no quoted real tool output backing it is a fabrication, even if it sounds plausible — this has happened in practice (a model claimed a fetch "succeeded" and described rich content for an issue that was verified separately to be empty). If genuine tool output cannot be quoted, the fetch did not succeed and must be reported as a failure, not summarized as if it were content.
