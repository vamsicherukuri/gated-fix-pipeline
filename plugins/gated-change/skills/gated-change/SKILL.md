...
1. Start from real GitHub issue context. The controller has no direct shell tool; three fetch paths exist, tried in order: (1) let `gated-change-intake` fetch it itself via `github/get_issue` and `github/get_issue_comments` — GitHub's built-in, out-of-the-box, read-only GitHub MCP server tools, documented as available by default for the `github-copilot` target, giving a direct tool call by the agent that needs the data instead of a delegated subagent narrating a result; (2) delegate to the named `gated-change-issue-fetcher` agent — a narrow, single-purpose agent whose only job is running one exact `gh` command and returning raw output verbatim; (3) ask the human to run `gh issue view --json ...` locally and paste the raw output — the only path proven reliable across repeated tests, and always an acceptable fallback. Fetches via paths 1 or 2 are treated as unverified until the human explicitly confirms they match the real issue; path 3 needs no extra confirmation since the human produced it directly. In practice, subagent fetches in this environment have been observed to fabricate complete, plausible-looking JSON for content that doesn't exist, so quoting the JSON is necessary but not sufficient proof on its own. Never reconstruct issue content from model memory or a chat attachment preview, even if the same issue was discussed earlier in the session. If the fetched (and, where required, human-confirmed) issue has no substantive content (blank/whitespace-only body, no comments — GitHub disallows a blank title, so a bare title alone never counts as substantive content), the controller stops and asks the human to add content to the issue itself — it does not invoke Intake for a content-empty issue.
