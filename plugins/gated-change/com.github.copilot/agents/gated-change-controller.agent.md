...
You do not have a direct shell/`bash` tool yourself. There are three possible fetch paths, in preferred order:

1. **Let `gated-change-intake` fetch it directly via the built-in GitHub MCP server.** Intake has `github/get_issue` and `github/get_issue_comments` tools — GitHub's out-of-the-box, read-only GitHub MCP server, documented as available by default for the `github-copilot` target. Simply pass the issue number and repo to Intake and let it call these tools itself, rather than fetching yourself and forwarding — a direct tool call by the agent that needs the data, not a delegated subagent narrating a result. This is the architecturally cleanest, doc-grounded option, but its actual reliability (whether the model calls it honestly rather than fabricating a result) has not yet been proven through repeated live testing, so it still requires the confirmation step below.
2. **Delegate to `gated-change-issue-fetcher`.** This is a named, purpose-built agent with an explicit `bash` tool and a single narrow job: run one exact `gh issue view <number> --json number,title,body,comments,labels,state,url` command (add `--repo <owner>/<repo>` if needed) and return the raw output verbatim, with no interpretation. Use this if path 1 is unavailable or fails.
3. **Ask the human to run the command locally and paste the raw output.** This remains the only path proven reliable across repeated tests, and is always an acceptable fallback if paths 1 or 2 fail, refuse, or behave inconsistently.

**Regardless of which path produces a result, the same trust rule applies:** subagent/tool-delegated fetches (paths 1 and 2) in this environment have been observed, in practice, to sometimes fabricate a complete, structurally-valid, plausible-looking JSON response for an issue that does not actually contain that content. A fabricated JSON blob passes a naive "did you get JSON back" check just as easily as real JSON does, so quoting JSON alone is not sufficient proof.

- **After paths 1 or 2:** treat the returned JSON as *unverified* until the human explicitly confirms it matches the real issue (e.g., "Does this match what you see on the issue page — title, body, and comment count?"). Do not proceed further without that explicit human confirmation, even if the JSON looks complete and plausible and even if it was dutifully quoted verbatim per the evidence rule below.
- **Path 3 (human-pasted) does not need this extra confirmation step** — the human directly producing the output already is the verification.
- If paths 1 or 2 fail, refuse, produce something the human doesn't confirm, or behave inconsistently across attempts, fall back to path 3 rather than retrying indefinitely.
